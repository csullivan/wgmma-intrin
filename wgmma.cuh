...
__device__ void ptx_wgmma_f16_m64n256k16_a_regs(float r[128], __nv_half2 a_regs[4], uint64_t desc_b) {
  int scale_d = false;
  int imm_scale_a = 1;
  int imm_scale_b = 1;
  int imm_trans_b = 0;
  asm volatile(
      ...
      : "=f"(r[0]), "=f"(r[1]), "=f"(r[2]), "=f"(r[3]), "=f"(r[4]), "=f"(r[5]), "=f"(r[6]), "=f"(r[7]),
        ...
        "=f"(r[120]), "=f"(r[121]), "=f"(r[122]), "=f"(r[123]), "=f"(r[124]), "=f"(r[125]), "=f"(r[126]), "=f"(r[127])
      :
        "r"(__HALF2_TO_UI(a_regs[0])), "r"(__HALF2_TO_UI(a_regs[1])), "r"(__HALF2_TO_UI(a_regs[2])), "r"(__HALF2_TO_UI(a_regs[3])),
        "l"(desc_b),
        "r"(scale_d),
        "r"(imm_scale_a),
        "r"(imm_scale_b),
...
__global__ void wgmma_f16_m64n256k16_register_layout_kernel(__nv_half* a, __nv_half* b, __nv_half* c) {
  ...
  float c_regs[128] = {0.0f};
  ptx_wgmma_f16_m64n256k16_a_regs(
      c_regs,
      a_regs2,
      descriptor((uint64_t)b_shared, 128, 128 * 16 / 8, 0, 0));
